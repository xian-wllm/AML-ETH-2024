"""
LOF BEST PARAMS + ALL OTHER BEST PARAMS from 8.py
...
def main():
    ...
    model = pipeline.make_pipeline(
        ensemble.StackingRegressor(
            estimators=[
                ("svr", svm.SVR(C=55.0, epsilon=1e-05)),
                ("gbm", ensemble.GradientBoostingRegressor(learning_rate=0.0925)),
...
def remove_outliers(X_train, y_test, X_test):
    model = pipeline.make_pipeline(
        ...
        LocalOutlierFactor(n_neighbors=20, contamination=0.0475)
    )
...
def select_features(X_train, y_train, X_test):
    model = pipeline.make_pipeline(
        ...
        feature_selection.SelectFromModel(linear_model.Lasso(0.095)),
    )
...
def create_submission(model, X_train, y_train, X_test):
    ...
    np.savetxt("submission_9_git_with_lof_best_params.csv", pred, delimiter=",", header="id,y", comments="", fmt=["%d", "%d"])
